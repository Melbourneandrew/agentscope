...
export const doctorReportSchema = z.strictObject({
  findings: z.array(doctorFindingSchema).max(256),
  fixed: z.boolean(),
  repairs: z.array(doctorRepairSchema).max(2),
  summary: z.strictObject({
    errors: z.number().int().nonnegative().max(256),
    information: z.number().int().nonnegative().max(256),
