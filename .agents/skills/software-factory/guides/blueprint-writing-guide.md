...
Use Blueprints only for durable internal architecture that implementations must preserve. Do not encode release milestones, alpha or version admission subsets, implementation or merge order, rollout stages, or project sequencing as ADRs; record those in ROADMAP or Beads. A delivery plan cannot authorize implementation to diverge from an approved Blueprint, and a Blueprint cannot waive external product truth owned by requirements.
