...
const pidOneChildren=()=>readFileSync("/proc/1/task/1/children","utf8").trim();const children=()=>readFileSync(\`/proc/\${process.pid}/task/\${process.pid}/children\`,"utf8").trim();const fds=()=>readdirSync("/proc/self/fd").length;const environment=extra=>[\`EXTRA_FD=\${extra}\`,"LANG=C","LC_ALL=C","PATH=/usr/local/bin:/usr/bin:/bin","TZ=UTC"];
...
const closeFinalizerToken=Symbol("agentscope-pty-close-finalizer");let closeFinalizerState="pending";let closeFinalizerCount=0;let closeFinalizerDropped=false;const closeRegistry=new FinalizationRegistry(value=>{closeFinalizerCount+=1;closeFinalizerState=closeFinalizerDropped&&value===closeFinalizerToken&&closeFinalizerCount===1?"exact":"invalid";});let closeLive=invoke(faults);for(const fd of [closeLive.p.interpreter,closeLive.p.script,closeLive.extra])closeSync(fd);faults.testFault(1<<5);const closeMessage=rejects(()=>faults.close(closeLive.result.handle),/close is uncertain/);rejects(()=>faults.inspect(closeLive.result.handle),/identity changed/);closeRegistry.register(closeLive.result.handle,closeFinalizerToken);const closeTerminalReceipt=closeLive.terminal;closeLive.result.handle=undefined;closeLive.result=undefined;closeLive=null;closeFinalizerDropped=true;const reuse=openSync("/dev/null",constants.O_RDONLY);const closeEnd=Date.now()+1000;while(Date.now()<closeEnd&&(closeFinalizerState==="pending"||closeTerminalReceipt()===undefined)){global.gc?.();await new Promise(resolve=>setTimeout(resolve,10));}let reuseLive=true;try{fstatSync(reuse);}catch{reuseLive=false;}closeSync(reuse);const closeTerminal=closeMessage.includes("uncertain")&&closeFinalizerState==="exact"&&closeFinalizerCount===1&&reuseLive&&closeTerminalReceipt()!==undefined;
